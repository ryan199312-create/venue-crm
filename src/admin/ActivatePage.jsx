...
const ActivatePage = () => {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setErr('');
    if (!identifier.trim()) return setErr('請輸入您的 Email 或電話號碼。');
    ...
    try {
      ...
      setErr(String(e2?.message || '啟用失敗，請重試。').replace(/^FirebaseError:\s*/, ''));
      setBusy(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-2xl shadow-xl p-8 text-center animate-in zoom-in-95">
          ...
          <h1 className="text-xl font-black text-slate-800 mb-2">請驗證您的 Email</h1>
          <p className="text-sm text-slate-500 font-medium mb-6 leading-relaxed">
            我們已寄出驗證信到 <span className="font-bold text-slate-700">{identifier}</span>。
            請點擊信中連結完成驗證。您現在可以先進入系統。
          </p>
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95">
        <div className="bg-slate-900 p-8 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h1 className="text-2xl font-black text-white tracking-tight">啟用您的帳戶</h1>
            <p className="text-indigo-300 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">Activate your account</p>
          </div>
        </div>

        <div className="p-8">
          <p className="text-sm text-slate-500 font-medium mb-6 leading-relaxed">
            請輸入管理員為您登記的 <b className="text-slate-700">Email 或電話號碼</b>，並設定您的密碼。
          </p>
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)}
                  ...
                  placeholder="name@company.com 或 91234567" required />
              </div>
            </div>

            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">設定密碼 (New Password)</label>
              <div className="relative group">
