...
const CustomBillingItemRow = React.memo(({ item, idx, onUpdate, onRemove }) => {
  return (
    <div className="grid grid-cols-12 gap-4 px-6 py-3 items-center hover:bg-slate-50 transition-colors border-t border-slate-50">
      ...
      <div className="col-span-2 flex items-center justify-end">
        ...
        <input type="number" value={item.price} onChange={e => onUpdate(idx, 'price', e.target.value)} className="w-20 text-right bg-transparent border-b border-slate-200 outline-none text-sm font-mono" />
      </div>
      <div className="col-span-2">
        <div className="flex items-center border border-slate-300 rounded-md bg-white h-9 overflow-hidden">
          <select value={item.unitType || 'fixed'} onChange={e => onUpdate(idx, 'unitType', e.target.value)} className="bg-slate-100 border-r border-slate-300 h-full px-2 text-[10px]"><option value="fixed">固定</option><option value="perTable">席</option></select>
          <input type="number" value={item.qty || ''} onChange={e => onUpdate(idx, 'qty', e.target.value)} className="w-full h-full text-center outline-none text-sm font-bold" />
        </div>
      </div>
      ...
      <div className="col-span-2 flex justify-center items-center gap-2">
        <label className="flex items-center text-[10px] font-bold text-slate-400 cursor-pointer hover:text-blue-600"><input type="checkbox" checked={item.isAddendum} onChange={e => onUpdate(idx, 'isAddendum', e.target.checked)} className="mr-1 rounded-sm" /> <span>附加</span></label>        <button 
          type="button"
          onClick={() => onUpdate(idx, 'applySC', item.applySC === false)}
          className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${item.applySC !== false ? 'text-blue-600 bg-blue-50' : 'text-slate-300'}`}
        >SC</button>
        <button type="button" onClick={() => onRemove(idx)} className="text-slate-300 hover:text-red-500 p-1"><Trash2 size={14} /></button>
      </div>
    </div>
  );
});
