...
const ReceiptView = ({ data, printMode }) => {
  ...
  return (
    <div className="font-sans text-slate-900 w-full max-w-[210mm] print:max-w-none mx-auto bg-white p-[10mm] print:p-0 min-h-[297mm] print:min-h-0 shadow-sm print:shadow-none relative flex flex-col text-xs leading-tight print-page">
      ...
      <DocumentHeader data={data} typeEn="RECEIPT" typeZh="收據" />
      ...
      <div className="flex justify-end mb-8 border-t border-slate-200 pt-6 relative z-10 break-inside-avoid">
        <div className="w-full md:w-1/2 lg:w-5/12 space-y-2.5">
          <div className="flex justify-between text-xs text-slate-600 px-4"><span>Grand Total (活動總額)</span><span className="font-mono font-medium">${formatMoney(billing.grandTotal)}</span></div>
          <div className="flex justify-between text-sm font-bold text-emerald-600 border-b border-slate-300 pb-3 px-4"><span>Total Received (已收總額)</span><span className="font-mono">${formatMoney(totalPaid)}</span></div>
          <div className="flex justify-between text-sm font-bold text-red-600 pt-2 px-4 bg-slate-50 py-2 rounded"><span>Balance Due (尚欠尾數)</span><span className="font-mono">${formatMoney(Math.max(0, billing.grandTotal - totalPaid))}</span></div>
        </div>
      </div>
...
const AddendumView = ({ data, printMode, onClientSign, onAdminSign }) => {
  const BRAND_COLOR = '#A57C00';
  const { clientSig, adminSig, sigData } = getSignatures(data, printMode);
        ...
        <table className="w-full text-xs text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            <tr className="bg-slate-50 border-b-4 border-slate-200">
              <td colSpan="3" className="py-2 px-4 text-right font-bold text-slate-600">Original Total (原合約總額):</td>
