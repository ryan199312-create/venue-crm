...
const PrintableEO = ({ data, printMode, appSettings, onClientSign, onAdminSign }) => {
  ...
  const isClient = printMode === 'QUOTATION' || printMode === 'CONTRACT' || printMode === 'CONTRACT_CN' || printMode === 'INVOICE' || printMode === 'RECEIPT' || printMode === 'MENU_CONFIRM';

  const shouldShow = (field, defaultClient, defaultInternal) => {
  ...
  if (printMode === 'BRIEFING') {
    return (
      <div className="font-sans text-slate-900 w-full bg-white p-6 relative">
        ...
        {/* --- CRITICAL ALERTS --- */}
        {( (data.specialMenuReq && shouldShow('specialMenuReq', false, true)) || (data.allergies && shouldShow('allergies', false, true)) ) && (
          <div className="mb-4 p-3 border-4 border-red-600 bg-red-50 rounded-lg flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-black text-red-900 leading-tight">
                {data.specialMenuReq && shouldShow('specialMenuReq', false, true) ? data.specialMenuReq + ' ' : ''} 
                {data.allergies && shouldShow('allergies', false, true) ? data.allergies : ''}
              </p>
            </div>
          </div>
        )}
            ...
            <div className="border-2 border-slate-200 rounded-xl overflow-hidden">
              ...
              <div className="p-3 space-y-3">
                {data.busInfo && data.busInfo.enabled ? (
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-indigo-50 p-2 rounded border border-indigo-100">
                      ...
                      {data.busInfo.arrivals && data.busInfo.arrivals.length > 0 ? data.busInfo.arrivals.map((b, i) => (
                    ...
                    <div className="bg-indigo-50 p-2 rounded border border-indigo-100">
                      ...
                      {data.busInfo.departures && data.busInfo.departures.length > 0 ? data.busInfo.departures.map((b, i) => (
                        <div key={i} className="font-mono font-bold">{b.time} <span className="font-sans font-normal text-[10px]">({b.location})</span></div>
                      )) : <span className="text-slate-400">-</span>}
                    </div>
                  </div>
                ...
                <div className="border-t border-slate-100 pt-2 flex justify-between items-center text-xs">
                  ...
                  <span className="font-bold bg-slate-100 px-2 py-0.5 rounded">{data.parkingInfo && data.parkingInfo.ticketQty || 0} Tickets ({data.parkingInfo && data.parkingInfo.ticketHours || 0} hrs)</span>
                </div>
              </div>
            </div>
              ...
              <div className="p-3 grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="col-span-2 p-2 bg-white border border-slate-200 rounded">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {Object.keys(avMap).map(k => data.avRequirements && data.avRequirements[k] && (
                ...
                {data.otherNotes && shouldShow('otherNotes', true, true) && (
  ...
  if (printMode === 'QUOTATION') {
    ...
    const getEquipmentEnStr = () => {
      let items = Object.entries(data.equipment || {}).filter(([k, v]) => v === true && typeof equipmentMap !== 'undefined' && equipmentMap[k]).map(([k]) => {
        ...
        return match ? match[1] : equipmentMap[k];
          ...
          if (data.equipment && data.equipment.nameSign && data.nameSignText) items.push(`Name Sign: ${data.nameSignText}`);
          if (data.equipment && data.equipment.hasCake && data.cakePounds) items.push(`Wedding Cake: ${data.cakePounds} Lbs`);
      return items.join(', ');
    };
    ...
    const getDecorEnStr = () => {
      let items = Object.entries(data.decoration || {}).filter(([k, v]) => v === true && typeof decorationMap !== 'undefined' && decorationMap[k]).map(([k]) => {
        ...
        return match ? match[1] : decorationMap[k];
          ...
          if (data.decoration && data.decoration.hasFlowerPillar && data.flowerPillarQty) items.push(`Floral Pillars: ${data.flowerPillarQty}`);
          if (data.decoration && data.decoration.hasMahjong && data.mahjongTableQty) items.push(`Mahjong: ${data.mahjongTableQty} sets`);
          if (data.decoration && data.decoration.hasInvitation && data.invitationQty) items.push(`Invitations: ${data.invitationQty} sets`);
          if (data.decoration && data.decoration.hasCeremonyChair && data.ceremonyChairQty) items.push(`Ceremony Chairs: ${data.ceremonyChairQty}`);
      return items.join(', ');
    };
    ...
    return (
      <div className="font-sans text-slate-900 max-w-[210mm] mx-auto bg-white p-6 min-h-[297mm] relative flex flex-col">
        <style>{`
            @media print { 
              @page { 
                margin: 10mm; 
        ...
        <div className="grid grid-cols-2 gap-8 mb-6">
          <div>
            ...
            {data.printSettings && data.printSettings.quotation && data.printSettings.quotation.showClientInfo !== false ? (
  ...
  if (printMode === 'CONTRACT' || printMode === 'CONTRACT_CN') {
    ...
    const getEquipmentStr = (en) => {
      let items = Object.entries(data.equipment || {}).filter(([k, v]) => v === true && typeof equipmentMap !== 'undefined' && equipmentMap[k]).map(([k]) => {
        ...
        return en && match ? match[1] : fullStr.split(' (')[0];
          ...
          if (data.equipment && data.equipment.nameSign && data.nameSignText) items.push(en ? `Name Sign: ${data.nameSignText}` : `字牌: ${data.nameSignText}`);
          if (data.equipment && data.equipment.hasCake && data.cakePounds) items.push(en ? `Cake: ${data.cakePounds} Lbs` : `蛋糕: ${data.cakePounds}磅`);
      return items.join(', ');
    };
    ...
    const getDecorStr = (en) => {
      let items = Object.entries(data.decoration || {}).filter(([k, v]) => v === true && typeof decorationMap !== 'undefined' && decorationMap[k]).map(([k]) => {
        ...
        return en && match ? match[1] : fullStr.split(' (')[0];
          ...
          if (data.decoration && data.decoration.hasFlowerPillar && data.flowerPillarQty) items.push(en ? `Pillars: ${data.flowerPillarQty}` : `花柱: ${data.flowerPillarQty}支`);
          if (data.decoration && data.decoration.hasMahjong && data.mahjongTableQty) items.push(en ? `Mahjong: ${data.mahjongTableQty}` : `麻雀: ${data.mahjongTableQty}張`);
          if (data.decoration && data.decoration.hasInvitation && data.invitationQty) items.push(en ? `Invitations: ${data.invitationQty}` : `喜帖: ${data.invitationQty}套`);
          if (data.decoration && data.decoration.hasCeremonyChair && data.ceremonyChairQty) items.push(en ? `Chairs: ${data.ceremonyChairQty}` : `婚椅: ${data.ceremonyChairQty}張`);
      return items.join(', ');
    };
    ...
    const SectionHeader = ({ title }) => (
      <div className="mb-4 mt-8">
        <h3 className="text-sm font-black uppercase tracking-widest pb-2 border-b-2 inline-block" style={{ color: BRAND_COLOR, borderColor: BRAND_COLOR }}>
          {title}
        </h3>
      </div>
    );
    ...
    return (
      <div className="font-sans text-slate-800 w-full bg-white p-8 relative text-sm leading-relaxed">
        <style>{`
          @media print { 
            @page { 
              margin: 12mm 15mm; 
            ...
            .legal-text { font-size: 11px; text-align: justify; line-height: 1.6; color: #334155; }
            .legal-header { font-weight: 800; margin-bottom: 4px; margin-top: 12px; font-size: 12px; color: #0f172a; }
          }
        `}</style>

        {/* --- HEADER --- */}
        <div className="flex justify-between items-end border-b-2 pb-4 mb-8" style={{ borderColor: BRAND_COLOR }}>
          <div>
            <div className="flex items-baseline gap-3 mb-2">
              <span className="text-4xl font-black tracking-tight leading-none" style={{ color: BRAND_COLOR }}>璟瓏軒</span>
              <span className="text-sm font-bold tracking-widest uppercase text-slate-500">King Lung Heen</span>
            </div>
            <div className="text-xs text-slate-500 font-medium uppercase tracking-tight">
              4/F, HK Palace Museum, West Kowloon <span className="mx-2">|</span> Tel: 2788 3939 <span className="mx-2">|</span> banquet@kinglungheen.com
            </div>
          </div>
          <div className="text-right">
            <div className="text-sm font-bold text-slate-700 font-mono tracking-tighter">REF: {data.orderId}</div>
            <div className="text-xs text-slate-500 uppercase tracking-widest mt-1">Date: {formatDateEn(new Date())}</div>
          </div>
        </div>

        {/* --- CENTERED WELCOMING SECTION --- */}
        <div className="mb-10 text-center mx-auto max-w-3xl">
          <h1 className="text-3xl font-light text-slate-800 uppercase tracking-[0.2em] mb-4">
          ...
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 grid grid-cols-2 gap-8">
            <div>
              ...
              <div className="space-y-5">
                ...
                {data.specialMenuReq && shouldShow('specialMenuReq', false, true) && (
                  <div className="pl-1 pt-3 border-t border-slate-200/50">
                ...
                {data.allergies && shouldShow('allergies', false, true) && (
                  <div className="pl-1 pt-3 border-t border-slate-200/50">
            ...
            <div>
              ...
              <div className="space-y-5 text-sm text-slate-700">
                ...
                <div className="border-b border-slate-200 pb-2">
                  ...
                  {data.venueDecor && shouldShow('venueDecor', false, true) && (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-slate-800 leading-tight"><span className="text-slate-500 mr-2">{isEn ? 'Free Parking:' : '免費泊車:'}</span> <span className="font-bold">{data.parkingInfo && data.parkingInfo.ticketQty || 0}</span> {isEn ? 'tickets' : '張'} x <span className="font-bold">{data.parkingInfo && data.parkingInfo.ticketHours || 0}</span> {isEn ? 'hrs' : '小時'}</p>
                  {data.busInfo && data.busInfo.enabled && (
                    <p className="text-slate-800 leading-tight">
                      <span className="text-slate-500 font-bold mr-2">{isEn ? 'Bus Arranged:' : '旅遊巴安排:'}</span> {data.busInfo.arrivals && data.busInfo.arrivals.length || 0} {isEn ? 'Arrival' : '接載'}, {data.busInfo.departures && data.busInfo.departures.length || 0} {isEn ? 'Departure' : '散席'}
                    </p>
                  )}
                  {data.otherNotes && shouldShow('otherNotes', true, true) && <p className="text-slate-800 leading-tight mt-2"><span className="text-slate-500 font-bold block mb-1">{isEn ? 'Remarks:' : '備註:'}</span> <span className="whitespace-pre-wrap">{data.otherNotes}</span></p>}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                ...
                {billing.setupPackagePrice > 0 && (
                  <div className="flex text-sm items-baseline text-slate-800">
                    <div className="flex-1 pr-4 font-medium">
                      ...
                      <div className="text-xs text-slate-500 font-normal leading-tight mt-1">{setupStr}</div>
                    </div>
                ...
                {billing.avPackagePrice > 0 && (
                  <div className="flex text-sm items-baseline text-slate-800">
                    <div className="flex-1 pr-4 font-medium">
                      ...
                      <div className="text-xs text-slate-500 font-normal leading-tight mt-1">{avStr}</div>
                    </div>
                ...
                {billing.decorPackagePrice > 0 && (
                  <div className="flex text-sm items-baseline text-slate-800">
                    <div className="flex-1 pr-4 font-medium">
                      ...
                      <div className="text-xs text-slate-500 font-normal leading-tight mt-1">{decorStr}</div>
                    </div>
              ...
              <div className="border-t border-slate-300 pt-4 flex justify-end">
                <div className="w-72 space-y-2 text-sm">
                  <div className="flex justify-between text-slate-600"><span>{isEn ? 'Subtotal' : '小計'}</span><span className="font-mono">${formatMoney(billing.subtotal)}</span></div>
                  {billing.serviceChargeVal > 0 && <div className="flex justify-between text-slate-600"><span>{isEn ? `Service Charge (10%)` : `服務費 (10%)`}</span><span className="font-mono">+${formatMoney(billing.serviceChargeVal)}</span></div>}
                  ...
                  <div className="flex justify-between items-center text-base font-black border-t-2 border-slate-900 pt-3 mt-3 text-slate-900 uppercase">
                    <span className="tracking-tight">{isEn ? 'Estimated Total' : '總金額'}</span>
                    <span className="font-mono text-xl">${formatMoney(billing.grandTotal)}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    ...
                    <tr className="text-sm">
                      ...
                      <td className="py-4 text-slate-900 font-mono font-bold underline decoration-slate-400 underline-offset-4">{balanceDueDateDisplay}</td>
                      <td className="py-4 text-right font-black text-slate-900 font-mono text-base underline decoration-slate-400 underline-offset-4">${formatMoney(billing.balanceDue)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        <div className="columns-2 gap-10 legal-text text-slate-700">
          ...
          <p className="mb-3">
            {isEn
              ...
              : "未經同意不得攜帶外部食物/飲品。裝飾必須僅使用“Blu-tack”。禁止吸煙。場地保留停止不安全活動或佈置的權利。場地佈置需活動前30天提交平面圖、施工細節，施工流程及物料清單等。若高於3米裝置，需提交RSE及有效保險單。客戶在簽署合約時，須支付押金作為保障，用於補償因客戶或其委託人員造成的場地及設施損壞或清潔費用（例如嘔吐清潔費）。押金金額HKD 10,000至50,000，若在活動結束後檢查確認未發生損壞，押金將在48小時全額退還；如有損壞或清潔需求，將從押金中扣除相應費用，剩餘部份則予以退還"}
          </p>
  ...
  if (printMode === 'INVOICE') {
    ...
    return (
      <div className="font-sans text-slate-900 w-full bg-white p-8 relative text-xs leading-tight">
        ...
        {/* Remarks Section */}
        {data.otherNotes && shouldShow('otherNotes', true, true) && (
          <div className="mb-6 bg-slate-50 p-4 rounded border border-slate-200 text-xs break-inside-avoid">
            <p className="font-bold text-slate-700 mb-1 uppercase">Remarks (備註)</p>
            <p className="text-slate-600 whitespace-pre-wrap">{data.otherNotes}</p>
          </div>
        )}
        ...
        {/* Bank Info */}
        <div className="mt-12 bg-slate-50 p-4 rounded border border-slate-200 text-xs break-inside-avoid">
      ...
      <div className="font-sans text-slate-900 w-full bg-white p-8 relative text-xs leading-tight">
        ...
        {isFullyPaid && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
            <span className="text-9xl font-black text-emerald-100 border-8 border-emerald-100 rounded-3xl px-12 py-6 rotate-[-15deg] tracking-widest">
              PAID
            </span>
          </div>
        )}
        ...
        {/* Remarks Section */}
        {data.otherNotes && shouldShow('otherNotes', true, true) && (
          <div className="mb-6 bg-slate-50 p-4 rounded border border-slate-200 text-xs relative z-10 break-inside-avoid">
            <p className="font-bold text-slate-700 mb-1 uppercase">Remarks (備註)</p>
            <p className="text-slate-600 whitespace-pre-wrap">{data.otherNotes}</p>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-8 items-stretch">
            ...
            <div className="flex flex-col justify-between">
              <div className="space-y-3">
                ...
                <span>上列內容有效期至: <span className="font-bold underline text-slate-900">{data.printSettings && data.printSettings.menu && data.printSettings.menu.validityDateOverride || defaultExpiry}</span></span>
                  </p>
                  <p className="flex items-start gap-1">
                    <span className="font-bold">•</span>
  ...
  if (!printMode || printMode === 'EO') {
    ...
    const getEquipmentStr = () => {
      ...
      if (data.equipment && data.equipment.nameSign && data.nameSignText) items.push(`字牌: ${data.nameSignText}`);
      if (data.equipment && data.equipment.hasCake && data.cakePounds) items.push(`蛋糕: ${data.cakePounds}磅`);
      return items.join(', ');
    };
    const getAVStr = () => {
    ...
    const getDecorStr = () => {
      ...
      if (data.decoration && data.decoration.hasFlowerPillar && data.flowerPillarQty) items.push(`花柱: ${data.flowerPillarQty}支`);
      if (data.decoration && data.decoration.hasMahjong && data.mahjongTableQty) items.push(`麻雀: ${data.mahjongTableQty}張`);
      if (data.decoration && data.decoration.hasInvitation && data.invitationQty) items.push(`喜帖: ${data.invitationQty}套`);
      if (data.decoration && data.decoration.hasCeremonyChair && data.ceremonyChairQty) items.push(`婚椅: ${data.ceremonyChairQty}張`);
      return items.join(', ');
    };
    ...
    return (
      <div className="font-sans text-slate-900 w-full bg-white text-sm">
        <style>{`
          @media print { 
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="border-2 border-slate-200 rounded-xl overflow-hidden">
                        ...
                        <div className="p-3 grid grid-cols-2 gap-2 text-xs">
                          ...
                          {data.venueDecor && shouldShow('venueDecor', false, true) && <div className="col-span-2 mt-2 p-2 bg-slate-50 rounded italic text-[10px] border border-slate-100"><span className="font-bold not-italic">佈置備註:</span> {data.venueDecor}</div>}
                          
                          {data.otherNotes && shouldShow('otherNotes', true, true) && (<div className="col-span-2 mt-2 pt-2 border-t border-slate-100"><span className="block text-[9px] text-red-500 font-bold uppercase mb-1">Remarks / Attention</span><p className="font-bold text-slate-900">{data.otherNotes}</p></div>)}
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="w-1/2 flex flex-col gap-4">
                        ...
                        <div className="border border-slate-300 rounded overflow-hidden break-inside-avoid">
                          ...
                          <div className="p-3 text-xs space-y-2">
                            ...
                            {data.venueDecor && shouldShow('venueDecor', false, true) && <div className="bg-slate-50 p-2 rounded italic text-[10px] border border-slate-100"><span className="font-bold not-italic">佈置備註:</span> {data.venueDecor}</div>}
                        ...
                        {data.otherNotes && shouldShow('otherNotes', true, true) && <div className="border border-yellow-200 bg-yellow-50 rounded p-2 text-xs"><span className="font-bold text-yellow-700 text-[10px] block uppercase">備註</span><p className="leading-tight text-yellow-900 whitespace-pre-wrap">{data.otherNotes}</p></div>}
                      </div>
                    </div>
                  )}
