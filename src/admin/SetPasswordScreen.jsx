  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (e2.code === 'auth/requires-recent-login') {
        setErr(L('基於安全考量，請重新登入後再設定密碼 (For security, please sign out and sign in again, then set your password).'));
      } else if (e2.code === 'auth/weak-password') {
        setErr(L('密碼強度不足 (Password too weak).'));
      } else {
        setErr(e2.message || L('設定失敗，請重試 (Something went wrong, please try again).'));
      }
      setBusy(false);
    }
  };
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95">
        <div className="bg-slate-900 p-8 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h1 className="text-2xl font-black text-white tracking-tight">{L('設定您的新密碼 (Set your new password)')}</h1>
            <p className="text-indigo-300 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">{L('設定您的新密碼 (Set your password)')}</p>
          </div>
        </div>

        <div className="p-8">
          <p className="text-sm text-slate-500 font-medium mb-6 leading-relaxed">
            {L('歡迎！為了帳戶安全，請將一次性密碼更換為您自己的密碼。 (Welcome! For account security, please replace the one-time password with your own.)')}
            {email && <span className="block mt-1 text-xs text-slate-400">{L('帳號 (Account)')}: <span className="font-bold text-slate-600">{email}</span></span>}
          </p>
          ...
          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">{L('新密碼 (New Password)')}</label>
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder={L('至少 8 位字元 (At least 8 characters)')}
                  required
                />
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder={L('再次輸入新密碼 (Re-enter new password)')}
                  required
                />
              </div>
            </div>
            ...
            >
              {busy ? <Loader2 className="animate-spin" size={20} /> : L('設定密碼並進入系統 (Set & Continue)')}
            </button>
          </form>
