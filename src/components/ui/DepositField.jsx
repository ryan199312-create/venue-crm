...
export const DepositField = ({ label, prefix, formData, setFormData, onUpload, addToast, onRemoveProof, clientPrefix, canConfirm, canEditPrice }) => {
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 ml-auto">
          ...
          {proofs.map((url, idx) => (
            <div key={idx} className="flex items-center space-x-1 bg-white px-2 py-1 rounded border border-slate-200 shadow-sm shrink-0">
              <a href={url} target="_blank" rel="noreferrer" className="flex items-center text-[10px] text-indigo-600 hover:underline truncate max-w-[120px]" title={getFileNameFromUrl(url)}>
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading || !canConfirm} className={`text-[10px] flex items-center bg-white px-2 py-1 rounded border transition-all shrink-0 shadow-sm ${!canConfirm ? 'text-slate-300 border-slate-100 cursor-not-allowed' : 'text-slate-500 hover:text-indigo-600 border-slate-200 hover:border-indigo-200 cursor-pointer'}`}>
          ...
          <label className={`flex items-center space-x-2 text-xs select-none shrink-0 ${!canConfirm ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}>
            <input type="checkbox" checked={formData[receivedKey] || false} disabled={!canConfirm} onChange={e => setFormData(prev => ({ ...prev, [receivedKey]: e.target.checked }))} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="relative">
          ...
          <input
            ...
            className={`w-full pl-8 pr-3 py-2 text-sm border border-slate-300 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none font-mono bg-white ${!canEditPrice ? 'text-slate-400 cursor-not-allowed' : ''}`}
            placeholder="0.00"
          />
        </div>
        <input
          ...
          className={`w-full px-3 py-2 text-sm border border-slate-300 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none bg-white ${!canEditPrice ? 'text-slate-400 cursor-not-allowed' : ''}`}
        />
      </div>
