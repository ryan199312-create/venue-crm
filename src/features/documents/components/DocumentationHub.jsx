...
const HELP_ARTICLES = [
  {
    ...
    description: '登入後的第一步，以及如何在系統中移動。', icon: Rocket, color: 'text-orange-500',
    content: `歡迎使用 VowsOS。
1. 使用左側導航列切換各個模組。
2.「儀表板」顯示今日活動與營收概覽。
3. 建議先完成三項基本設定：品牌與標誌、分店資料、以及新增團隊用戶（皆在「設定」中）。`
  },
  {
    id: 'branches', category: 'settings', title: '分店管理 (Branches)',
    description: '新增與管理旗下場地／分店。', icon: Building2, color: 'text-emerald-500',
    content: `在「設定 → 分店管理」中新增分店（門市）。
您可建立的分店數量受您方案的授權上限限制（畫面會顯示「已用 / 授權」）。
每個分店可各自設定專屬的場地資料、菜單與收款方式。`
  },
  {
    id: 'users', category: 'team', title: '團隊與用戶 (Users)',
    description: '登記員工帳戶，他們自行啟用並設定密碼。', icon: Users, color: 'text-blue-500',
...
可登記的用戶數量受方案授權上限限制。`
  },
  {
    ...
    description: '建立活動、菜單、流程與備註。', icon: FileText, color: 'text-indigo-500',
    content: `點擊「訂單管理 → 新增訂單」。填寫基本資料後，可接著設定菜單、場地佈置、物流與備註。
每張訂單皆可一鍵生成報價單、合約、宴會通知單 (EO) 與收據。`
  },
  {
    id: 'documents', category: 'finance', title: '單據與範本 (Documents)',
    description: '單據如何套用您的品牌與條款。', icon: BookOpen, color: 'text-violet-500',
    content: `報價單、合約與收據會自動套用您在「設定 → 分店資料」中的資訊：
場地名稱、標誌、地址、電話、付款條款、收款方式與報價單保密聲明。
所有這些內容皆按分店獨立設定——不同分店的單據會顯示各自的品牌，不會互相混用。`
  },
  {
    id: 'billing', category: 'finance', title: '財務與收款 (Billing)',
    description: '訂金、餘額追蹤與收款方式。', icon: CreditCard, color: 'text-rose-500',
    content: `系統會依菜單價格與人數自動計算總額。
在訂單的「財務」分頁記錄訂金與餘額支付，並生成 PDF 收據。
收款方式（銀行轉帳、FPS、支票等）於「設定 → 分店資料 → 付款方式設定」中維護。`
  },
  {
    id: 'client-portal', category: 'basics', title: '客戶專屬平台 (Client Portal)',
    description: '讓客戶以電話號碼查看自己的活動。', icon: Smartphone, color: 'text-teal-500',
    content: `客戶可透過您的客戶平台，以其電話號碼登入，查看活動流程、確認細節、上傳付款證明並線上簽署文件——無需帳號密碼。`
  },
  {
    id: 'ai-assistant', category: 'ai', title: 'AI 智慧助理 (AI Assistant)',
    description: '協助撰寫電郵草稿、分析數據與備註。', icon: Sparkles, color: 'text-fuchsia-500',
    content: `在訂單編輯頁或數據分析中，點擊 AI 圖標即可呼叫助理，協助產生電郵草稿、撰寫備註或分析營運數據。`
  }
];
...
const CATEGORIES = [
  ...
  { id: 'settings', label: '設定', icon: Building2 },
  { id: 'events', label: '訂單與活動', icon: FileText },
  { id: 'finance', label: '財務與單據', icon: CreditCard },
...
const DocumentationHub = () => {
  ...
  const filteredArticles = HELP_ARTICLES.filter(art => {
    ...
    const matchesSearch = art.title.toLowerCase().includes(q) || art.description.toLowerCase().includes(q);
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            {appSettings?.venueProfile?.nameZh || 'VowsOS'} 幫助中心 — 快速掌握系統操作。
          </p>
        </div>
        <div className="relative w-full md:w-80">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
          <input type="text" placeholder="搜尋教學文章..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredArticles.map(art => (
          <Card key={art.id} className="p-6 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group border-slate-200" onClick={() => setSelectedArticle(art)}>
            ...
            <p className="text-sm text-slate-500 leading-relaxed font-medium mb-6">{art.description}</p>
            <div className="flex items-center justify-end pt-4 border-t border-slate-50">
              <span className="text-xs font-bold text-indigo-600 flex items-center gap-1 group-hover:gap-2 transition-all">閱讀 <ChevronRight size={16} /></span>
            </div>
          </Card>
        ))}
        {filteredArticles.length === 0 && (
          <div className="col-span-full text-center py-16 text-slate-400 italic">找不到相關文章。</div>
        )}
      </div>
      ...
      <div className="bg-slate-900 rounded-3xl p-10 text-white relative overflow-hidden shadow-xl">
        ...
        <div className="relative z-10 max-w-2xl space-y-5">
          <h3 className="text-3xl font-black tracking-tight leading-tight">需要更多協助？<br /><span className="text-indigo-400">我們的支援團隊隨時待命。</span></h3>
          <p className="text-slate-400 font-medium leading-relaxed">
            如在使用上遇到任何問題，或希望客製化流程，歡迎聯絡 VowsOS 支援團隊。
          </p>
          <a href="mailto:hello@vowsos.com" className="inline-flex bg-white text-slate-900 px-8 py-3 rounded-2xl font-black items-center gap-2 hover:scale-105 transition-all active:scale-95 shadow-xl">
            <MessageCircle size={18} /> 聯絡支援 (Contact Support)
          </a>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300" onClick={() => setSelectedArticle(null)}>
          <Card className="max-w-2xl w-full p-10 bg-white max-h-[80vh] overflow-y-auto relative animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">{CATEGORIES.find(c => c.id === selectedArticle.category)?.label || selectedArticle.category}</span>
                <h3 className="text-3xl font-black text-slate-800">{selectedArticle.title}</h3>
              </div>
            </div>
            <p className="text-base text-slate-600 leading-relaxed font-medium whitespace-pre-line">{selectedArticle.content}</p>
            <div className="mt-12 pt-8 border-t border-slate-100 flex justify-end">
              <button onClick={() => setSelectedArticle(null)} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold">完成閱讀 (Close)</button>
            </div>
          </Card>
        </div>
      )}
