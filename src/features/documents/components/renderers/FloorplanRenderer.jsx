...
export const FloorplanAppendix = ({ data, appSettings, isStandalone = false }) => {  
  ...
  // SMART ORIENTATION: Maximize printable area
  const isHorizontal = contentWidth > contentHeight;
  
  // Base printable area (A4 minus margins and header)
  const portraitWidth = isStandalone ? 700 : 660;
  const portraitHeight = isStandalone ? 700 : 660;

  // If horizontal, we swap target dimensions because we will rotate 90deg
  const targetWidth = isHorizontal ? portraitHeight : portraitWidth;
  const targetHeight = isHorizontal ? portraitWidth : portraitHeight;
  ...
  // Resulting dimensions after scaling (but before rotation)
  const finalW = contentWidth * scale;
  const finalH = contentHeight * scale;
  ...
  return (
    <React.Fragment>
      ...
      {/* 
        CONTAINER: 
        If rotated, the height of the container must be the WIDTH of the content.
      */}
      <div 
        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl overflow-hidden mt-4 relative shadow-sm flex items-center justify-center mx-auto" 
        style={{ 
          height: isHorizontal ? `${finalW + 40}px` : `${finalH + 40}px`, 
          breakInside: 'avoid' 
        }}
      >
        {/* Visual Bounding Box Wrapper: Corrects layout overflow by providing exact visual dimensions */}
        <div 
          className="flex items-center justify-center flex-shrink-0"
          style={{
            width: isHorizontal ? `${finalH}px` : `${finalW}px`,
            height: isHorizontal ? `${finalW}px` : `${finalH}px`,
            position: 'relative'
          }}
        >
          <div 
            className="relative transition-transform duration-500 flex-shrink-0" 
            style={{ 
              width: `${finalW}px`,
              height: `${finalH}px`,
          ...
          >
            ...
            {zones.length > 0 && (
                <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
                  {zones.map(z => {
                    ...
                    const points = z.points.map(p => `${(p.x_m * itemScale) - minX},${(p.y_m * itemScale) - minY}`).join(' ');
                    const cx = (((Math.min(...z.points.map(p => p.x_m)) + Math.max(...z.points.map(p => p.x_m))) / 2) * itemScale) - minX;
                    const cy = (((Math.min(...z.points.map(p => p.y_m)) + Math.max(...z.points.map(p => p.y_m))) / 2) * itemScale) - minY;
            ...
            {(fp.elements || []).map(el => {
              ...
              return (
                <div key={el.id} className={`absolute flex items-center justify-center ${displayStyle}`} style={{ left: (el.x || 0) - minX, top: (el.y || 0) - minY, width: w_m * itemScale, height: h_m * itemScale, transform: `rotate(${el.rotation || 0}deg)` }}>
                  {el.type === 'text' ? (
