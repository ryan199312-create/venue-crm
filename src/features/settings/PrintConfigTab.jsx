    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <div className="flex items-center gap-2 border-b border-slate-100 pb-3 mb-4">
          ...
          <h4 className="font-bold text-slate-800">{L('列印設定 (Print Customization)')}</h4>
        </div>

        <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
          <h5 className="font-bold text-indigo-800 text-sm mb-3">{L('通用設定 (General Settings)')}</h5>
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1">{L('覆蓋發單日期 (Issue Date Override)')}</label>
            <input 
              type="date" 
            ...
            />
            <p className="text-[10px] text-slate-500 mt-1">{L("留空則使用系統當前日期 (Leave blank to use today's date)")}</p>
          </div>
        </div>

        <div className="bg-violet-50 p-4 rounded-lg border border-violet-100">
          <h5 className="font-bold text-violet-800 text-sm mb-3">{L('菜單確認書 (Menu Confirmation)')}</h5>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">{L('覆蓋有效期日期 (Validity Date Override)')}</label>
              <input
                type="text"
                placeholder={L('預設: 今日+14天 (Default: Today+14)')}
        ...
        <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100">
          <h5 className="font-bold text-emerald-800 text-sm mb-3">{L('報價單 (Quotation)')}</h5>
          <label className="flex items-center space-x-3 cursor-pointer">
            ...
            <span className="text-sm font-bold text-slate-700">{L('顯示客戶資訊 (Show Client Info)')}</span>
          </label>
        </div>
      </div>
    </div>
