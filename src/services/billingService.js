...
/**
 * Robustly checks if a zone is selected, matching both legacy names and new combined labels.
 */
export const isZoneSelected = (zone, selectedLocations) => {
  if (!selectedLocations || !Array.isArray(selectedLocations)) return false;
  const combined = `${zone.nameZh}${zone.nameEn ? ` (${zone.nameEn})` : ''}`;
  return selectedLocations.includes(combined) || selectedLocations.includes(zone.nameZh);
};

/**
 * Normalizes a location selection to the current preferred label format.
 */
export const getPreferredZoneLabel = (zone) => {
  return `${zone.nameZh}${zone.nameEn ? ` (${zone.nameEn})` : ''}`;
};
